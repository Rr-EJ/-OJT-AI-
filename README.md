...
현대 조직에서는 잦은 인사 이동, 신입사원 채용, 외주/계약 인력 투입이 늘어나면서
새로운 구성원이 복잡한 사내 시스템에 빠르게 적응시키는 것이 중요한 과제로 떠오르고 있습니다.

그러나 실제 현장에서는
PPT 매뉴얼, 위키, 구두 안내, 화면 캡처 문서에 의존한 온보딩 방식 때문에
	•	“어디를 눌러야 할지 모르는” UI 불안
	•	사수에게 반복해서 물어봐야 하는 비효율
	•	Jira, Gmail, 사내 웹사이트 등 여러 시스템을 왔다 갔다 해야 하는 피로감

이 계속 발생하고 있습니다.

이 한계를 해결하기 위해,
신입사원이 자연어로 질문하면 업무 화면 위에서 길을 안내해 주고,
필요한 **메일·티켓·문서 작업을 AI 에이전트가 함께 처리해 주는
온보딩 특화 AI 비서 “AI MATE”**의 필요성이 대두되었습니다.
...
신입·전배자 모두의 적응 시간을 단축
	•	📚 지식 전달 표준화
사수·팀마다 다른 설명 대신, AI MATE가 최신 프로세스 기준으로
동일한 절차와 메시지를 안내하여 업무 품질 균일화
	•	🤝 협업 효율 향상
Gmail, Jira, Web Guide를 하나의 패널에서 연결하여
이슈 공유·문의 응답·피드백 과정을 매끄럽게 지원
	•	🚀 신입사원 온보딩 경험 개선
“실수할까 봐 무서운” 초기 불안을 줄이고,
문서 기반 Q&A와 실시간 화면 가이드를 통해
자기주도적으로 업무를 익힐 수 있는 환경 제공
